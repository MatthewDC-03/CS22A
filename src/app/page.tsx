...
export default function Home() {
    ...
    useEffect(() => {
        ...
        return () => {
            window.removeEventListener("scroll", handScroll); // Cleanup event listener
        };
    }, []);
    ...
    return (
        <>
            ...
            <main className="w-full h-full relative flex flex-col gap-20" ref={container}  >
                ...
                {/* Navbar */}
                <header className={`navbar w-full ${isMobile
                    ? "bg-[#1E1E1E] h-16" // Disable for mobile
                    : scrolled
                        ? "bg-[#1E1E1E] h-12"
                        : "bg-[black]/20 h-16 backdrop-blur-sm"} fixed top-0 left-0 z-50 max-sm:fixed max-sm:top-0 max-sm:left-0 max-sm:right-0`}>
