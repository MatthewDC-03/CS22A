    ...
    return (
        ...
        <div id="memberContainer" className="w-full h-full relative">
            ...
            <div id="cardContainer" className="absolute p-7 left-0 grid place-items-center grid-cols-3 z-30 top-0 w-full -skew-y-3 h-full" >
            {
                Data.map(item => {
                    return(
                        <div id={`${item.gsap.name}`} key={item.id} className="relative w-[350px] px-4 pb-4 flex flex-col gap-3 justify-around pt-[100px] items-center rounded-lg translate-x-0 scale-0 h-[500px] bg-[#F6F6F6] shadow-2xl" >
                            <div className=" h-40 w-40 rounded-full bg-red-500 overflow-hidden left-1/2 top-1/2 absolute -translate-x-1/2 -translate-y-80 border-4 border-[#F6F6F6]" >
                                <Image src={item.source} width={1000} height={1000} className="h-full w-full bg-cover" alt="..." ></Image>
                            </div>
                            <h1 className="text-3xl font-bold text-[#1E1E1E]" >Daisy</h1>
                            <p className="w-full text-sm text-center font-thin" >
                            ...
                            <div className="flex flex-col w-full text-xs tracking-widest font-bold h-fit gap-2">
